<template>
    <Teleport to="body">
        <div class="fixed inset-0 bg-black/75 backdrop-blur-xs z-100 flex items-end justify-center"
            ...
            <div class="bg-bg-surface border-t border-border rounded-t-[20px] w-full max-w-140 max-h-[88dvh] flex flex-col animate-slide-up"
                    ...
                    <div class="flex flex-col gap-1.5 relative">
                        ...
                        <div v-if="locationPickerOpen"
                            ...
                            <div class="bg-bg-surface border-t border-border rounded-t-[20px] w-full max-w-140 flex flex-col animate-slide-up"
                                ...
                                <!-- Sheet header -->
                                <div
                                    class="flex items-center justify-between px-5 pt-2 pb-4 border-b border-border-subtle shrink-0">
                                ...
                                <div class="flex-1 overflow-y-auto overscroll-contain">
                                    <div v-for="loc in filteredLocations" :key="loc.id"
                                        ...
                                        :class="editForm.locationId === loc.id
                                            ? 'bg-accent-dim text-accent'
                                            : 'text-text-secondary hover:bg-bg-elevated hover:text-text-primary active:bg-bg-hover'"
                                        :style="{ paddingLeft: `${16 + loc.indent * 18}px` }"
                                        @click="selectEditLocation(loc)">
                                        <IconCornerDownRight v-if="loc.indent > 0" :size="12"
                                            class="text-text-muted shrink-0" />
                                        <span class="font-['DM_Mono'] text-[13px] flex-1">{{ loc.name }}</span>
                                        <IconCheck v-if="editForm.locationId === loc.id" :size="15"
